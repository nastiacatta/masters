...
export default function ResultsPage() {
  ...
  const expAccuracyBar = useMemo(() =>
    [...methodAgg].filter((m) => m.deltaCrps != null)
      .sort((a, b) => (a.deltaCrps ?? 0) - (b.deltaCrps ?? 0))
      .map((m) => ({ name: m.label, method: m.method, deltaCrps: m.deltaCrps as number, color: m.color })),
    [methodAgg]);
        ...
        <section aria-label="Headline answers">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <AnswerCard
              ...
              metricLabel={useExp ? '\u0394CRPS vs equal (mechanism)' : '\u0394 mean error vs equal'}
        ...
        <section>
          ...
          {useExp && activeTab === 'Accuracy' && (
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <p className="text-[11px] text-slate-500 mb-3"><strong>Verdict:</strong> Skill \u00d7 stake is favourable if its bar is below 0.</p>
