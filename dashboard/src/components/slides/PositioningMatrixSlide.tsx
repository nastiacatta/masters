 ...
 * Nodes rendered as large rounded-rect cards with descriptions.
...
export default function PositioningMatrixSlide() {
  ...
  // Quadrant tint colours (very subtle)
  const quadrantTints = {
  ...
  return (
    <SlideShell
      ...
      refText="[3] Lambert et al., 2008  [4] Raja et al., 2024  [5] Vitali & Pinson, 2025"
    >
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        >
          ...
          {NODES.filter(n => !n.isThesis).map((node) => {
            ...
            // Shorten arrow so it doesn't overlap the card
            const dx = toX - fromX;
            const dy = toY - fromY;
          ...
          {/* Nodes as rounded-rect cards — MUCH bigger */}
          {NODES.map((node) => {
