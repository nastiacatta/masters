...
export const MAIN_DECK_SLIDE_COUNT = 17 as const;
 ...
 * 17-slide narrative: slide 5 = Mechanism Comparison, slide 14 = benchmark comparison slide.
 */
export const SECTIONS = {
  PROBLEM:    { label: 'PROBLEM',    colour: '#003E74',    slides: [1, 2, 3, 4, 5, 6] },
  SOLUTION:   { label: 'SOLUTION',   colour: '#2E8B8B',    slides: [7, 8, 9] },
  VALIDATION: { label: 'VALIDATION', colour: '#7C3AED',    slides: [10, 11, 12, 13, 14, 15, 16] },
  CLOSING:    { label: '',           colour: 'transparent', slides: [17] },
} as const;
...
export function getSectionForSlide(slideNumber: number): { label: string; colour: string } {
  ...
  if (slideNumber >= 10 && slideNumber <= 16) return SECTIONS.VALIDATION;
