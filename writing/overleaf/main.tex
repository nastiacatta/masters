...
approximately eleven percentage points of CRPS; this gap quantifies the
...
eleven percentage points of CRPS on the full-length expanding-mode run.
...
Horizon-specific experiments under the older warmup-window normalisation
provide a sanity check on the mechanism's behaviour away from the
one-step-ahead headline.
Table\textasciitilde{}\ref{tab:horizon-day-ahead} reports the day-ahead
horizon (\(h = 24\), warmup at least \(70\)).
...
is \(0.10835\) (\(-0.36\%\)), the mechanism is \(0.10808\)
(\(-0.61\%\)), and rolling best-single is \(0.10388\) (\(-4.48\%\)). On
a within-run seasonal slice, uniform is \(0.06716\), the mechanism is
\(0.06646\) (\(-1.05\%\)), and rolling best-single is \(0.05980\)
(\(-10.97\%\)). The per-season breakdown is uniformly positive at
approximately \(+1\%\) CRPS improvement, with no season falling below
the baseline.
...
list of remaining work; the direction of the comparisons is stable and
the magnitudes are expected to shift by under one percentage point.
...
\citet{raja2024wagering} on the warmup-window normalisation.
...
The rolling isotonic recalibration layer closes \(59\%\) of the tail
calibration gap at a \(1.3\%\) CRPS cost and an \(11\%\) sharpness cost,
...
comparison is \(t = 40.77\) on the full-length wind run, well inside any
...
\(74.0\)\textasciitilde MW, without using weather inputs. The mechanism,
aggregating a panel that mixes the best forecaster with weaker models,
reaches \(83.7\)\textasciitilde MW, approximately \(13\%\) above Elia's
...
(,\(-18.0\%\)) and the rolling best-single selector (,\(-22.9\%\)) beat
...
closes \(59\%\); the remaining \(41\%\) is the cost of the linear-pool
form.
