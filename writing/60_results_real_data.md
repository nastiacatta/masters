...
This section reports empirical results on two Elia series:
offshore-wind power and electricity-imbalance prices. Two wind
slices are analysed in parallel with distinct purposes. The
\emph{headline slice} covers the full 17,344-hour run under
strictly-causal expanding normalisation; it is the primary
method-versus-method aggregation comparison. The \emph{audit
slice} is a 3,000-point subset under the older warmup-window
normalisation; it is kept as the reference slice for per-quantile
calibration analysis and for cross-checking against the published
OGD reference.
...
On the headline wind slice, the mechanism reduces mean CRPS against
uniform averaging by $7.1\%$. Table~\ref{tab:wind-headline} reports
mean Continuous Ranked Probability Score (CRPS) on the normalised
$[0, 1]$ scale for the full method suite.
...
averaging is $t = 22.35$ (\citet{andrews1991hac} data-driven HAC bandwidth,
selected lag $12$) with $p \approx 0$. Under the legacy
horizon-$1$ HAC bandwidth, which is uncorrected for persistence in
the loss differential, the same statistic inflates to $t = 40.77$.
The skill-only rule against uniform gives $t = 21.30$ (Andrews) and
$t = 38.92$ (legacy). The paired mechanism-versus-uniform
difference is $-7.1\%$ with $p \approx 0$ under both bandwidths. A
95\% block-bootstrap confidence interval on
$\mathbb{E}[\Delta\mathrm{CRPS}]$ at block size $168$ (one week of
hourly data) gives $[-0.003214, -0.002605]$ for the mechanism,
well separated from zero. All subsequent DM statistics in this
chapter use the Andrews-auto bandwidth unless stated. The legacy
values are also emitted into \texttt{comparison.json} as a
sensitivity check.
...
($-7.1\%$ against $-7.0\%$): the skill gate's contribution on top
of Bates--Granger-style inverse-error weighting is within noise on
this slice. The mechanism against the shifted-median fan is
$-7.1\%$ against $-14.5\%$; the published OGD reference baseline
beats the mechanism by $7.4$ percentage points of CRPS, the price
paid for keeping the Lambert budget-balance constraint. The
mechanism against the per-round oracle ($-7.1\%$ against $-46.7\%$)
leaves an oracle gap of roughly forty percentage points, which no
self-financed aggregator can close; the mechanism captures about
$15\%$ of the available oracle gap. The mechanism against the
rolling best-single selector ($-7.1\%$ against $-22.9\%$) loses by
...
The rolling best-single selector is the forecaster with the
lowest recent average CRPS over a 100-step lookback window. It is
not a per-round oracle; the per-round argmin row at the top of the
table is the oracle.

The mechanism and the trimmed-mean baseline differ by $0.1$
percentage point of CRPS ($-7.1\%$ against $-7.2\%$). Under the
...
The closeness of trimmed-mean and mechanism on this slice reflects
the absence of adversarial reports in the Elia panel: the skill
gate has no adversarial tail to discard and therefore no
structural advantage over robust averaging.
...
Two features of the $7.1\%$ CRPS reduction matter more than its
magnitude. First, the gain coincides, to within statistical noise,
with Bates--Granger-style inverse-CRPS weighting ($-7.0\%$), a
baseline that is itself beaten by the unweighted median ($-9.3\%$)
on the same slice. The adaptive skill layer is therefore a minor
source of the improvement on this panel: almost all of the gain
is accessible to any aggregator that uses even a crude proxy for
historical reliability. Read against \citet{timmermann2006forecast}
and \citet{magnus2023inconvenient}, this is the forecast-
combination-puzzle regime. Once a panel is heterogeneous enough
...
through the wager pool (the economic layer) rather than to act as
a separate weighting module bolted on top. The oracle gap is
wide: the per-round argmin oracle reaches $0.02176$ on the same
slice, about forty percentage points below uniform averaging. The
...
considered here, is the information that a single round's data
...
without being told a priori which model is best.

#### External validation against the Elia operational forecast

Table~\ref{tab:elia-operational} compares the mechanism and the
best single forecaster against Elia's own published operational
forecasts. CRPS is expressed in megawatt-equivalent units using
the observed series range ($0$ to $2\,208.7$\,MW).
...
its native nine-point grid, which compares two different Riemann
approximations of the same CRPS integral. The pinball integrand
is piecewise-linear in $\tau$, and on this series it is locally
concave between grid points; the three-point trapezoidal rule
systematically under-integrates relative to the nine-point rule,
giving Elia an artificial $\sim 20\%$ grid advantage. The revised
table below re-scores Elia's fan on the same nine-level grid by
linear interpolation between $(c_{10},\,\mathrm{mean},\,c_{90})$,
projected onto the monotone cone. The native three-grid Elia CRPS
is reported as a sensitivity check.
...
On the matched nine-level grid, a simple online XGBoost trained
on the observed series alone beats Elia's real-time forecast by
roughly $23\%$ in CRPS-megawatt-equivalent ($69.5$~MW against
$90.7$~MW), despite using no weather inputs. The mechanism
aggregates seven forecasters of varying quality and reaches
$83.7$~MW, which is $7.7\%$ below Elia's real-time forecast.
Elia's day-ahead forecast, a more realistic forward operational
product, is considerably weaker at $121.2$~MW; the mechanism
improves on it by about $31\%$.

Elia's published interval forecasts are systematically
miscalibrated. Nominal $\tau = 0.10$ gives empirical coverage of
$19.1\%$, and $\tau = 0.90$ gives $94.6\%$. The miscalibration is
a known property of operational numerical-weather-prediction
forecasts and motivates the recalibration layer developed in
Section~\ref{ch:recalibration} as a generic operational tool.

\paragraph{Best-single ceiling and the conditional reading of the
headline.} The seven-forecaster aggregate lands at $83.7$~MW,
while the best single member (online XGBoost) reaches $69.5$~MW.
The aggregate is therefore bounded below by its weakest members:
a panel in which one forecaster does most of the work is, by
construction, one on which an aggregator that cannot concentrate
weight enough will fail to match the best single model. This is
the best-single-ceiling observation of
\citet{timmermann2006forecast} in direct form, and it motivates
the conditional reading of the $-7.1\%$ headline. The thesis does
not claim to beat operational NWP-driven forecasting or the best
single model. It claims a conditional improvement over uniform
averaging inside the budget-balanced design space.

\paragraph{Hyperparameter sensitivity.} Parameters tuned for
synthetic panels with roughly ten forecasters and $T \approx 1{,}000$
rounds are not optimal on the wind series, which prefers a more
aggressive $(\gamma, \rho)$. The held-out sensitivity sweep
reported later in this section selects the wind-specific values;
a poorly tuned skill gate leaves the mechanism marginal against
uniform. The headline number is stable to $\gamma$ and $\rho$
mis-specification but depends materially on keeping $\lambda$ small.

### Elia offshore wind: audit slice

The audit slice covers the first $3\,000$ evaluation points under
the older warmup-window causal normalisation, on the post-fix
pipeline that clips negative raw wind values to zero before
normalisation. The audit run itself uses expanding rather than
static normalisation because the warmup of this slice (winter
wind) has a systematically higher range than the evaluation
window. Static normalisation would clip about $46\%$ of evaluation
...
The ratio of mechanism to shifted-median-fan baseline is
$0.02000 / 0.02030 = 0.985$: the mechanism reduces CRPS against
the reference by $1.5\%$ on this slice. The Diebold--Mariano
statistic for the mechanism against uniform is $t = +8.26$
(\citet{andrews1991hac} auto HAC bandwidth, selected lag $8$), $p \approx 0$;
the legacy horizon-$1$ bandwidth gives $t = +15.43$.

#### Per-forecaster CRPS on the audit slice

Table~\ref{tab:wind-audit-per-agent} reports per-forecaster CRPS,
which supplies the per-agent ranking used for the rank-correlation
diagnostics.
...
The Spearman rank correlation between the learned $\sigma$ and
...
The recalibration layer in Section~\ref{ch:recalibration} corrects
the shift.
...
CRPS ($0.01775$ against $0.02000$, a gap of about $11\%$). The
CRPS advantage comes from relaxing the Lambert budget-balance
constraint and learning per-$\tau$ weights directly. The
recalibration layer closes most of the centre deviation in
...
200-round warmup), under the same seven forecasters, mechanism
parameters, and expanding causal normalisation as the wind
headline. On this series the mechanism is a statistical null.
Table~\ref{tab:electricity-null} reports the aggregate comparison.
...
The Diebold--Mariano statistic for the mechanism against uniform
is $t = 0.007$ with $p = 0.994$ (Andrews auto HAC, lag $11$); the
$95\%$ block-bootstrap confidence interval on
$\Delta\mathrm{CRPS}$ at the 168-hour block size is
$[-0.000127, +0.000123]$ and straddles zero. The mechanism is not
statistically distinguishable from uniform on electricity. The
seven forecasters produce near-identical CRPS within about $1\%$
of each other, so the skill signal has no persistent structure to
exploit. The electricity result is the forecast-combination-puzzle
regime \citep{bates1969combination, timmermann2006forecast}, and
the mechanism behaves accordingly: a null rather than a
regression.

The oracle gap on electricity is about thirty-five percentage
points (mechanism $0.09052$, per-round oracle $0.05924$), so a
perfect per-round weight could gain a great deal. It will not be
found by an EWMA or OGD on this forecaster panel, because the
forecasters themselves are undifferentiated.

### Horizon experiments

Horizon-specific experiments are a sanity check on the mechanism's
...
($-1.75\%$), and rolling best-single is $0.10230$ ($-4.76\%$). On
a within-run seasonal slice, uniform is $0.06541$, the mechanism
is $0.06309$ ($-3.55\%$), and rolling best-single is $0.05729$
($-12.42\%$). The per-season breakdown falls between $-3.2\%$ and
$-4.3\%$ against the uniform baseline in every season (winter
$-4.3\%$, spring $-3.2\%$, summer $-3.4\%$, autumn $-3.3\%$), and
no season rises above uniform.

These horizon experiments use the default warmup-window (static)
normalisation. The magnitudes shift only slightly under the
expanding variant on the main slice; re-running the horizon blocks
under expanding is on the list of remaining work. The direction of
the comparisons is stable.

\paragraph{Hyperparameter provenance.} Earlier revisions of these
tables were regenerated before the audit-M3 fix, at which point
the \texttt{\_run\_horizon\_comparison} call path silently dropped
...
stronger in magnitude. The post-M3 code takes
$(\gamma, \rho, \lambda)$ as keyword arguments with the tuned
values as defaults, so the caveat is lifted on the numbers shown
here.

### Threats to validity

\paragraph{Internal validity.} An earlier pipeline used
whole-series min-max normalisation, a non-reproducible
neural-network seed, tail-adjacent cross-validation for XGBoost,
and a silent persistence-fallback path. The current pipeline
...
(about $125$ days) and is roughly stationary. Full-year drift is
not tested in the headline. The panel has seven members. Results
at $n = 50$ or $n = 500$ are not directly extrapolable.

\paragraph{Construct validity.} CRPS is strictly proper
\citep{gneiting2007strictly} but aggregates calibration and
sharpness in a specific way: a mechanism optimised for CRPS can
be sub-optimal for a decision that weights tail coverage or point
accuracy differently. Headline comparisons run against multiple
baselines (uniform, inverse-CRPS, median, trimmed mean, rolling
best-single, the \citet{vitali2025intermittent} reference), so
the claim is not anchored to a single reference point.

\paragraph{Statistical validity.} The Diebold--Mariano test
assumes covariance stationarity of the loss differential. HAC
standard errors are used, but residual non-stationarity is not
ruled out. No family-wise error correction is applied across the
ten-method, two-dataset, two-horizon design. The headline
...
the mechanism on both series, by about $14$ percentage points on
wind and $4$ percentage points on electricity. The gap is the
CRPS cost of preserving the Lambert budget-balance and per-round
truthfulness guarantees; Vitali's aggregator drops both in
exchange for CRPS.

### Sensitivity sweep and parameter provenance

Tuned parameters come from the held-out sweep described in
Chapter~\ref{ch:mechanism}, not from hand-tuning.
...
band of $-0.17$ to $-0.22\%$, consistent with the null reported
above.

Re-running the headline comparison under expanding normalisation
at sweep-selected parameters is on the list of remaining work.
The expected shift is sub-percent CRPS in either direction. The
...
this is a narrow peak, in which case a small mis-tuning would
collapse the headline improvement, or a broad plateau, in which
case the $7.1\%$ result is stable to modest parameter noise. A
denser local sweep on
$\gamma \in \{12, 16, 20, 24, 28, 32, 40, 48\}$,
$\rho \in \{0.3, 0.4, 0.5, 0.6, 0.7, 0.8\}$, and
$\lambda \in \{0.03, 0.05, 0.08, 0.12\}$ answers the question,
scoring each cell on the same held-out test partition as the main
sweep.

The best cell is $(\gamma, \rho, \lambda) = (28, 0.8, 0.03)$ at a
test CRPS change of $-7.69\%$, marginally better than the
coarse-grid value of $-6.86\%$. Twenty-one of $192$ cells ($11\%$)
fall within $0.5$ percentage points of the optimum, and $60\%$
fall within $2$~pp. The optimum is a broad plateau. Axis-wise, the
mechanism's sensitivity to $\lambda$ is the largest of the three
parameters ($-7.69\%$ at $\lambda = 0.03$ rising to $-5.50\%$ at
$\lambda = 0.12$, holding $\gamma, \rho$ at their optima), while
$\gamma$ is stable across a wide band and CRPS falls monotonically
with more aggressive $\rho$ up to the grid ceiling of $0.8$. The
plateau shape localises the tuning risk: the headline number is
stable to $\gamma$ and $\rho$ mis-specification but depends
materially on keeping $\lambda$ small.
...
which means part of the reported improvement could be
cross-regime adaptation rather than within-regime skill. To
separate the two effects the mechanism is re-run with a full
restart at each seasonal boundary (winter, spring, summer,
autumn), resetting wealth, $\sigma$, and the EWMA loss buffers at
the start of each season. Table~\ref{tab:regime-shift-restart}
reports the per-season mechanism improvement against uniform.
