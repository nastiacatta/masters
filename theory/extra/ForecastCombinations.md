...
arXiv:2205.04216v2  [stat.ME]  23 Sep 2022
...
Initial work on forecast combinations after the seminal work of Bates and Granger (1969) focused
...
2.1
Simple point forecast combinations
...
to time T, ˆyT+h|T =
  ˆyT+h|T,1, ˆyT+h|T,2, . . . , ˆyT+h|T,N
′, into a single combined h-step-ahead forecast
˜yT+h|T = C
 ˆyT+h|T; wT+h|T

, where N is the number of forecasts to be combined and wT+h|T is an
...
,
(1)
where ΣT+h|T is the N × N covariance matrix of the h-step forecast errors and 1 is an N-dimensional
unit vector. This is implemented, for example, in the R package ForecastComb (Weiss et al., 2018). In
practice, the elements of the covariance matrix ΣT+h|T are usually unknown and need to be estimated.
...
weights. Additionally, Aiolﬁand Timmermann (2006) argued in favor of clustering the individual
...
due to difﬁculties in properly estimating the covariance matrix ΣT+h|T, especially in situations with
...
the combination weights when ΣT+h|T is unknown, are extended to the general case as follows:
wbg1
T+h|T,i =

∑T
...
Correlations across forecast errors are either ignored by treating the covariance matrix ΣT+h|T as a
diagonal matrix or estimated via the usual sample estimator (which may lead to quite unstable esti-
mates of ΣT+h|T given highly correlated forecast errors). Some estimation schemes suggest computing
...
Aiolﬁand Timmermann (2006) constructed the weights proportional to the inverse of performance
...
0
+ T ˆΣ−1
/(ν0 + T) is the precision matrix and ˆΣ is the sample covariance matrix.
...
purely linear combinations (k = N and p = 0) and nonlinear combinations (k = 0 and p ̸= 0).
...
Adhikari (2015) deﬁned the nonlinear term using vij =
 ˆzi −mij ˆzj
  ˆzj −mji ˆzi

, where ˆzi denotes the
...
2.5
Which forecasts should be combined?
...
3.7
Combinations constructed via Bayes’ Theorem
...
a normal distribution with mean ˜µ = 1′Σ−1µ/1′Σ−11 and variance ˜σ2 = 1/1′Σ−11, where 1 is an
...
combination of the individuals’ means with weights 1′Σ−1/1′Σ1 identical to the “optimal” weights
...
pooling methods.
3.8
Combinations constructed via integration
...
 1, x′
T+h
′ and θT+h = (θT+h,0, θT+h,1, . . . , θT+h,N)′. A dynamic linear model is built to
