...
\end{enumerate}
\begin{itemize}
\item \textbf{budget balance}: $\sum_i \pi_i(t) = \sum_i b_i(t)$ (self-financing); in this project we split it into a refund and a skill payoff, see §[Slide 7](\#slide-7);
...
\item \textbf{Privacy.} Participants reveal a predictive distribution, not the underlying data or model. This is exactly the positioning argued in Raja et al. (2024, §1–§2) and reused here.
\end{itemize}
...
\item Raja et al. (2024, IJF), §3 — the \textbf{buyer/seller + aggregation operator + scoring rule + payoff} architecture we directly reuse.
\end{itemize}
...
\subsection{Why equal weights is the natural naïve baseline}
...
\subsection{Why strategic behaviour breaks naïve averaging}
...
\item \textbf{History.} A truly adversarial forecaster performs badly under the proper scoring rule over time; the \emph{skill gate} converts their persistently high loss into a small effective wager, muting their influence (this is what §[Slide 8](\#slide-8) formalises).
\end{enumerate}
...
\subsection{The 2×2 positioning matrix (rendered by \texttt{presentation/R/plot\\_positioning\\_matrix.R})}
...
The table is a React component: \texttt{dashboard/src/components/slides/MechanismComparisonSlide.tsx}. The Vitali row deliberately says "Shapley payoffs (not self-financed)" — we use the exact wording from §III of the 2025 paper.
...
\item \textbf{Pre-round.} $\sigma_i(t)$ depends only on realised losses up to $t-1$. So the truthfulness argument from Lambert extends directly: at round $t$, a risk-neutral agent who takes $m_i(t)$ as given maximises expected payoff by reporting truthfully. Details in §[Appendix A](\#appendix-a).
\end{itemize}
...
\midrule
Submit & "decide how much to deposit" & `staking.choose\_deposits` (staking.py:61) & See `theory/Pierre\_wagering.md` Raja §3.1 \\
Skill gate & "multiply deposit by a factor" & `staking.skill\_gate` (staking.py:91); `effective\_wager\_bankroll` (staking.py:100) & Eq. 6 in Lambert; extended with $\sigma$ here \\
Aggregation & "effective wagers as weights" & `aggregation.aggregate\_forecast` & Raja §3.2 (forecast aggregation operator) \\
Settlement & "Lambert weighted-score formula" & `settlement.settle\_round` (settlement.py:67) & Lambert (2008) Thm. 1 \\
Update & "skill estimate updates" & `skill.update\_ewma\_loss`, `skill.loss\_to\_skill` & CRPS-based online learning, Pinson et al. \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{CRPS-hat ≠ exact CRPS.} We use a finite-quantile approximation on a non-equidistant grid $(0.1, 0.25, 0.5, 0.75, 0.9)$. The approximation is good but not exact; it is called "CRPS-hat" throughout the code (\texttt{scoring.py:83–103}).
\end{itemize}
...
Code: \texttt{onlinev2/src/onlinev2/behaviour/…} and \texttt{simulation.py} \texttt{deposit\\_mode}.
...
\item \textbf{Bankroll × Confidence.}
\end{enumerate}
...
\textbf\{Bankroll × Conf\} & \textbf\{0.0279\} & \textbf\{−0.0145\} & \textbf\{−34 \%\} \\
Oracle (upper bound) & 0.024 & −0.0184 & −43 \% \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Takeaway.} Bankroll × Confidence closes most of the oracle gap; the mechanism "looks good" only when deposits themselves carry useful signal. The slide badge therefore reads "Bankroll+Conf: −34\% relative to Fixed".
...
Driver: \texttt{scripts/run\\_real\\_data\\_with\\_skill.py} → \texttt{onlinev2.real\\_data.runner.run\\_real\\_data\\_comparison}. For each dataset:

\begin{itemize}
\item $n = 7$ forecasters from §[Slide 9](\#slide-9), strictly causal.
...
\begin{itemize}
\item \textbf{Raja ≪ This project.} The learning layer is what produces the big improvement over Raja on wind (−44 \% vs −2 \%). This is direct empirical evidence that \emph{history-free self-financed} is leaving a large gain on the table.
...
\item Deposit design: bankroll × confidence closes ~75 \% of the oracle gap over fixed deposits.
...
\item \textbf{Results page} — pick Elia wind → scroll mean CRPS by method → optional skill trajectories.
...
All dashboard pages read \textbf{the same JSON} the slides are built from (\texttt{dashboard/public/data/…}), so anything on the slides can be reproduced live.
...
\item \texttt{src/components/slides/…} — per-slide components.
\item \texttt{public/data/real\\_data/…/comparison.json}, \texttt{.../baselines.json}.
