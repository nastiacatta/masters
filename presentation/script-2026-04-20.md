...
* Lambert: self-financed wagering — seven formal properties, uniqueness
...
The agent layer generates per-round actions. On the real data, seven forecasting models participate: Naive persistence, EWMA with span five, ARIMA, XGBoost, a multi-layer perceptron, the Theta method, and a Naive-plus-EWMA ensemble. All models are strictly causal — they use only data up to time t minus one to predict time t. Quantiles are generated via residual bootstrap with isotonic monotonicity enforcement.
...
The core pipeline runs each round in five steps: submit, skill gate, aggregate, settle, update. The same effective wager determines both aggregation weight and financial exposure. Budget balance holds by construction — total payouts always equal total effective wagers.
...
The graph shows the learned skill for each forecaster over time. The mechanism correctly identifies Naive persistence as the strongest forecaster — it achieves the highest skill — and ARIMA as the weakest. The other five models spread out in between. This is the mechanism learning in action on real data.
...
A skill-only variant already improves over equal weighting by about thirty per cent. The full mechanism — coupling skill with the deposit and settlement structure — improves further. That supports the claim that learned reliability is genuinely useful for aggregation.

At the same time, the best single forecaster still outperforms the aggregate. That is an important point to state clearly. The contribution is conditional improvement, not universal dominance.
...
Skill recovery: in the controlled benchmark with six forecasters of known noise levels, the learned ranking matches the true ranking perfectly. Spearman rank correlation is one point zero zero zero zero.
