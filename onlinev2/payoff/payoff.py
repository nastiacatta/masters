...
def aggregate_forecast(reports, m, alpha=None, eps=1e-12, fallback=None):
    """
    Quantile averaging (QA): q_hat(tau) = sum_i w_i * q_i(tau),
    where w_i = m_i / sum_j m_j.
        ...
        if fallback is not None:
            ...
            if reports.ndim > 1:
                if out.shape != reports[0].shape:
                    raise ValueError(f"fallback shape {out.shape} != reports[0].shape {reports[0].shape}")
            else:
                if out.size != 1:
                    raise ValueError(f"fallback must be scalar for point mode, got size {out.size}")
