...
def cap_weight_shares(
    ...
    """
    Cap per-agent weight share at omega_max, then rescale to preserve sum(m).

    After capping, sum(m_cap) == sum(m_original).
    ...
    Returns
    -------
    m_cap : (n,) capped + rescaled wagers
    """
    ...
    shares = m_t / M
    shares = np.minimum(shares, float(omega_max))
