...
# Pipeline-version tag embedded in forecast caches (see clause 1.3 / 2.3 of
# .kiro/specs/model-training-testing-audit/bugfix.md). Caches without this
...
def normalize_series(series: np.ndarray) -> tuple[np.ndarray, float, float]:
    ...
    headline CRPS figures (bugfix clause 1.1). It is retained only for
...
def causal_normalize(
    ...
    This is the headline fix for bugfix clauses 1.1 / 2.1 /
    2.2 in ``.kiro/specs/model-training-testing-audit/``.
...
def best_single_by_crps(
    ...
    definition across runners (bugfix clauses 1.11 / 2.11).
...
def run_real_data_comparison(
    ...
    # Spec: .kiro/specs/mechanism-recalibration-layer/ (requirements.md, design.md).
    rc = None
    ...
    for t in range(warmup, T):
        ...
        # Shared helper with _run_horizon_comparison (bugfix clause 1.11 / 2.11).
        if t - warmup >= 20:
    ...
    # implementation is flagged as follow-up in
    # .kiro/specs/model-training-testing-audit/tasks.md (optional 11.7).
    ...
    try:
        ...
        panel_pt = q_reports[:, warmup:T, tau_mid_idx].T  # shape (T_eval, N)
        y_eval = y_all[warmup:T]
        try:
            mich = _michael_run(panel_pt, y_eval, {"quantile": 0.5, "eta": 0.01})
            for idx in range(len(per_round)):
                y_t = float(y_all[warmup + idx])
                y_hat_t = float(mich["y_hat"][idx])
    ...
    # --- Online-run per-block mean CRPS (bugfix clause 1.14 / 2.14) ---
    ...
    # and is flagged as follow-up (see tasks.md optional item 11.8).
    K_preq = 5
    ...
    # --- Online-run windowed mean (bugfix clause 1.13 / 2.13) ---
    ...
    # each train/test split and is flagged as follow-up (see tasks.md
    # optional item 11.6 for the dashboard rename and the stronger
    # implementation).
    nreps = 10
    train_frac = 0.6
    test_frac = 0.1
