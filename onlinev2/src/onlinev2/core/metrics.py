"""
Logging and metrics for the mechanism.

Adds:
  F1. PIT (probability integral transform) and sharpness logging
  F2. Concentration metrics: HHI, N_eff, Gini
  F3. Network export for collusion signatures
"""
...
def compute_pit(
    ...
    If the forecast is well calibrated, PIT should be ~ Uniform(0,1).
    """
...
def compute_sharpness(
    ...
    *,
    ...
    """
    Sharpness proxies:
      - interval_width: q(tau_H) - q(tau_L)
      - iqr: q(0.75) - q(0.25)
      - entropy_proxy: sum of log-differences between adjacent quantiles
    """
    quantiles = np.asarray(quantiles, dtype=np.float64).ravel()
    taus = np.asarray(taus, dtype=np.float64).ravel()
...
# ---------------------------------------------------------------------------
# F2. Concentration metrics
# ---------------------------------------------------------------------------

def compute_hhi(weights: np.ndarray) -> float:
    """
    Herfindahl–Hirschman Index: H_t = sum_i w_{i,t}^2.
...
class NetworkExporter:
    """
    Builds a similarity graph in rolling windows for collusion detection.

    Node features: mean stake, stake variance, participation rate,
                   median report, report width
    Edge weights: report correlation, stake correlation,
    ...
    def export_csv(self, output_dir: str, prefix: str = "network") -> None:
        """Export adjacency + features to CSV."""
        ...
        with open(node_path, "w", newline="") as f:
            ...
            for aid, feat in features.items():
                writer.writerow({"agent_id": aid, **feat})

        edges = self.compute_edge_weights()
...
class RoundMetricsLogger:
    """
    Accumulates per-round metrics for PIT, sharpness, concentration.

    Call log_round() after each round to record metrics.
    """

    def __init__(self, taus: Optional[np.ndarray] = None) -> None:
