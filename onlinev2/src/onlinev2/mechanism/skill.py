"""
Online skill estimation via EWMA loss tracking and loss-to-skill mapping.

L_t tracks exponentially weighted *normalised* loss per forecaster (always in [0,1]).
sigma_t = f(L_t) maps accumulated loss to a skill score in [sigma_min, 1].

Loss normalisation (E1):
  MAE mode:  loss already in [0,1] for y,r in [0,1]
  CRPS-hat mode: loss in [0,2], so feed L = loss/2 to keep EWMA in [0,1]

Gamma calibration (E2):
  Given a reference point (L_ref, sigma_ref), gamma is set analytically:
    gamma = -ln((sigma_ref - sigma_min) / (1 - sigma_min)) / L_ref

Missingness revert (E3):
  When kappa > 0 and L0 is set, absent forecasters' L decays toward L0.
  L0 is derived from a chosen baseline sigma_0:
    L0 = -ln((sigma_0 - sigma_min) / (1 - sigma_min)) / gamma
"""
import numpy as np
...
def update_ewma_loss(L_prev, losses_t, alpha_t, rho, kappa=0.0, L0=0.0):
    ...
    Default kappa=0 freezes the loss estimate while a forecaster is absent.
    """
...
def calibrate_gamma(sigma_ref, sigma_min, L_ref):
    ...
    This removes arbitrary tuning: pick a reference point like
    "after accumulating normalised loss L_ref=0.5, skill should be sigma_ref=0.5"
    and gamma follows.
    """
...
def missingness_L0(sigma_0, sigma_min, gamma):
    ...
    which means their skill reverts toward sigma_0.
    """
