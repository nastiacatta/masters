...
requirement for median-type forecasts (absolute-loss style). Use this when
forecasters report a single point (e.g. median or point estimate), not when
they report probabilities.
...
Brier and log score are the standard strictly proper scores for probability
forecasts; CRPS is strictly proper for probabilistic forecasts over outcomes.
...
def score_mae(outcome, report):
    ...
    Elicits a point forecast interpreted as a median-type report. s = 1 - |y - r|.
    Since |y - r| in [0,1], s in [0,1] by construction. No clip needed for settlement.
    For probability forecasts use quantiles_crps (CRPS) instead.
    """
    outcome = np.asarray(outcome, dtype=np.float64)
    report = np.asarray(report, dtype=np.float64)
